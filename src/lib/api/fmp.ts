...
export class FMPPlanError extends Error {
  constructor(endpoint = "unknown", reason?: string) {
    super(`FMP endpoint '${endpoint}' is not available on the current plan${reason ? ` (${reason})` : ""}`);
...
export const fmpClient = {
  ...
  async getQuote(symbols: string | string[]): Promise<FMPQuote[]> {
    ...
    const fetchSingleQuote = async (symbol: string): Promise<FMPQuote[]> => {
      try {
        return await fmpFetch<FMPQuote[]>(`${FMP_STABLE}/quote?symbol=${symbol}&apikey=${getApiKey()}`);
      } catch (error) {
    ...
    try {
      ...
      const settledQuotes = await Promise.allSettled(cleanedSymbols.map((symbol) => fetchSingleQuote(symbol)));
  ...
  async getProfile(symbol: string): Promise<FMPCompanyProfile[]> {
    return fmpFetch<FMPCompanyProfile[]>(`${FMP_STABLE}/profile?symbol=${symbol}&apikey=${getApiKey()}`, 3600);
  },
  ...
  async getMarketMovers(type: "gainers" | "losers" | "actives"): Promise<FMPMover[]> {
    const path = type === "gainers" ? "biggest-gainers" : type === "losers" ? "biggest-losers" : "most-actives";
   ...
   */
  async getNews(
    options: { type?: "general" | "stock" | "crypto" | "forex"; symbol?: string; limit?: number; page?: number } = {},
   ...
   */
  async getStockHistory(symbol: string, from: string, to: string): Promise<FMPHistoricalWrapper> {
   ...
   */
  async getInsiderTrades(symbol: string, limit = 50, page = 0): Promise<FMPInsiderTrade[]> {
    const normalizedSymbol = symbol.toUpperCase();

    const normalizeTrade = (trade: any): FMPInsiderTrade => ({
      ...trade,
      acquistionOrDisposition: trade.acquistionOrDisposition ?? trade.acquisitionOrDisposition ?? "",
   ...
   * Endpoint: /v3/search?query=AAPL&limit=10
   */
  async searchSymbols(query: string, limit = 10): Promise<FMPSearchResult[]> {
    const API_V3 = "https://financialmodelingprep.com/api/v3";
    const encoded = encodeURIComponent(query);
    return fmpFetch<FMPSearchResult[]>(`${API_V3}/search?query=${encoded}&limit=${limit}&apikey=${getApiKey()}`, 60);
  },
};

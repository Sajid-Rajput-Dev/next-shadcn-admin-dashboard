...
function InsiderRow({ trade }: { trade: FMPInsiderTrade }) {
  ...
  return (
    <TableRow>
      ...
      <TableCell className="font-bold font-mono">{trade.symbol}</TableCell>
      <TableCell className="max-w-[160px] truncate text-sm">{trade.reportingName}</TableCell>
      <TableCell className="text-muted-foreground text-sm">{trade.typeOfOwner}</TableCell>
      <TableCell className={`font-semibold ${typeColor}`}>{trade.transactionType}</TableCell>
      <TableCell>
        <Badge
          variant={isBuy ? "default" : "destructive"}
          className={`text-xs ${isBuy ? "bg-green-500 hover:bg-green-600" : ""}`}
        >
          {isBuy ? "Buy" : "Sell"}
        </Badge>
      </TableCell>
      <TableCell className="text-right text-sm">{trade.securitiesTransacted.toLocaleString()}</TableCell>
      <TableCell className="text-right text-sm">{trade.price > 0 ? `$${trade.price.toFixed(2)}` : "—"}</TableCell>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm">SEC Form 4 filings — executive and director buy/sell activity.</p>
      </div>
        ...
        <CardContent>
          <Table>
            ...
            <TableBody>
              ...
              {!isLoading &&
                !isError &&
                (data?.data as FMPInsiderTrade[])?.map((trade, i) => <InsiderRow key={i} trade={trade} />)}
            </TableBody>
          </Table>
          ...
          {!isLoading && data?.data && data.data.length >= 50 && (
            <div className="flex justify-center gap-2 pt-4">
              ...
              <span className="flex items-center text-muted-foreground text-sm">Page {page + 1}</span>

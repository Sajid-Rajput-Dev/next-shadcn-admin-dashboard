...
import { formatUSD, formatCompact } from "@/lib/utils/format-currency";
...
export function WhaleTable() {
  ...
  const filters = {
    ...
    minAmountUsd: parseInt(searchParams.get("min_amount") || "0"),
                ...
                <TableRow key={i}>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2 text-xs">
                      <span className="text-muted-foreground truncate max-w-[80px]" title={tx.from_owner_type || "Unknown"}>
                        {tx.from_owner_type || "Unknown"}
                      </span>
                      <ArrowRight className="h-3 w-3 text-muted-foreground" />
                      <span className="text-muted-foreground truncate max-w-[80px]" title={tx.to_owner_type || "Unknown"}>
                        {tx.to_owner_type || "Unknown"}
                      </span>
                    </div>
                  </TableCell>
                  <TableCell className="text-right text-muted-foreground">
                    {formatTradeDate(tx.timestamp)}
                  </TableCell>
                </TableRow>
              ))
            )}

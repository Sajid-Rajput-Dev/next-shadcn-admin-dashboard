...
import { useMarketNews, type NewsType } from "@/hooks/use-market-news";
                ...
                <div className="flex gap-4 p-4">
                    ...
                    <div className="flex flex-1 flex-col gap-1 overflow-hidden">
                        ...
                        <div className="mt-auto flex flex-wrap items-center gap-2">
                            <span className="text-xs text-muted-foreground">{article.site}</span>
                            <span className="text-xs text-muted-foreground">·</span>
                            <span className="text-xs text-muted-foreground">{date}</span>
                            {article.symbol && (
                                <Badge variant="outline" className="h-4 px-1 text-xs font-mono">
                                    {article.symbol}
                                </Badge>
                            )}
                            {article.sentiment && (
                                <span
                                    className={`rounded-full px-2 py-0.5 text-xs font-medium ${SENTIMENT_COLORS[article.sentiment] || SENTIMENT_COLORS.Neutral}`}
                                >
                                    {article.sentiment}
                                </span>
                            )}
        ...
        <div className="space-y-6">
            <div>
                ...
                <p className="text-muted-foreground text-sm">
                    Latest headlines from FMP — refreshed every 5 minutes.
                </p>
            </div>
            ...
            <div className="grid gap-3">
                ...
                {!isLoading &&
                    !isError &&
                    data?.data.map((article, i) => <NewsCard key={i} article={article} />)}
            </div>
        </div>
    );
}

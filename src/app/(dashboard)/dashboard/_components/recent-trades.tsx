...
export function RecentTrades() {
  ...
  const trades = showCrypto ? whaleData?.data : congressData?.data;

  return (
    <Card className="col-span-1 border-white/5 bg-card/50 backdrop-blur-sm lg:col-span-4">
      <CardHeader className="flex flex-row items-center justify-between">
        <CardTitle className="font-bold text-white text-xl">Recent Activity</CardTitle>
        <MarketToggle />
      </CardHeader>
      <CardContent>
        {isLoading ? (
          ...
          <div className="p-8 text-center text-muted-foreground">No recent trades found.</div>
        ) : (
          <div className="relative overflow-x-auto">
            <Table>
              <TableHeader className="bg-white/5">
                <TableRow className="border-white/5 hover:bg-transparent">
                  ...
                  <TableHead className="hidden text-right text-muted-foreground md:table-cell">Date</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                      ...
                      <TableCell className="font-medium">
                        <div className="flex items-center gap-2">
                          ...
                          <div className="flex flex-col">
                            <span className="font-medium text-foreground text-sm">{entityName}</span>
                            {trade.party && <span className="text-muted-foreground text-xs">{trade.party}</span>}
                          </div>
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className="flex flex-col">
                          <span className="flex items-center gap-1 font-bold">{ticker}</span>
                          <span className="text-muted-foreground text-xs">
                            {trade.asset_type || (showCrypto ? "Crypto" : "Stock")}
                          </span>
                        </div>
                      </TableCell>
                      <TableCell>
            ...
            <div className="mt-4 flex justify-end">
              <Link
                ...
                className="flex items-center text-primary text-sm transition-colors hover:text-primary/80"
              >

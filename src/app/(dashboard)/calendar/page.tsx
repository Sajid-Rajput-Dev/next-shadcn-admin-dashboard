...
import { type CalendarType, useEarningsCalendar } from "@/hooks/use-earnings-calendar";
import type { FMPDividendEvent, FMPEarningsEvent } from "@/lib/api/fmp";
...
function EarningsRow({ event }: { event: FMPEarningsEvent }) {
  const beat = event.eps !== null && event.epsEstimated !== null ? event.eps >= event.epsEstimated : null;

  return (
    <TableRow>
      <TableCell className="font-medium">{event.date}</TableCell>
      <TableCell className="font-bold font-mono">{event.symbol}</TableCell>
      <TableCell>
...
function DividendRow({ event }: { event: FMPDividendEvent }) {
  return (
    <TableRow>
      ...
      <TableCell className="text-muted-foreground text-sm">{event.declarationDate}</TableCell>
    </TableRow>
  );
}
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm">Upcoming earnings releases and dividend payments.</p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex gap-1">
          {[1, 2, 4].map((w) => (
            <Button key={w} variant={weeks === w ? "secondary" : "ghost"} size="sm" onClick={() => setWeeks(w)}>
              {w}w
            </Button>
          ))}
        </div>
            ...
            {type === "earnings" ? (
              <>
                ...
                <TableBody>
                  ...
                  {!isLoading &&
                    !isError &&
                    (data?.data as FMPEarningsEvent[])?.map((ev, i) => <EarningsRow key={i} event={ev} />)}
                </TableBody>
              </>
            ) : (
              <>
                ...
                <TableBody>
                  ...
                  {!isLoading &&
                    !isError &&
                    (data?.data as FMPDividendEvent[])?.map((ev, i) => <DividendRow key={i} event={ev} />)}
                </TableBody>
              </>
            )}
          </Table>

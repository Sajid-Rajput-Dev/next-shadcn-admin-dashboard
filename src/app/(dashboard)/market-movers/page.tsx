...
import { useMarketMovers, type MoverType } from "@/hooks/use-market-movers";
...
function MoverRow({ mover, type }: { mover: FMPMover; type: MoverType }) {
    ...
    return (
        <TableRow>
            <TableCell className="font-mono font-bold">{mover.symbol}</TableCell>
            <TableCell className="max-w-[200px] truncate text-sm text-muted-foreground">{mover.name}</TableCell>
            ...
            <TableCell>
                <Badge variant="outline" className="text-xs">{mover.exchange}</Badge>
            </TableCell>
        </TableRow>
    );
}
                    ...
                    <Table>
                        ...
                        <TableBody>
                            ...
                            {!isLoading &&
                                !isError &&
                                data?.data.map((mover) => (
                                    <MoverRow key={mover.symbol} mover={mover} type={activeTab} />
                                ))}
                        </TableBody>
                    </Table>
                </CardContent>

...
export async function POST(request: Request) {
  try {
    ...
    if (senateTrades.length > 0) {
      ...
      const { error, count } = await supabase.from("congress_trades").upsert(senateRows as any, {
    ...
    if (houseTrades.length > 0) {
      ...
      const { error, count } = await supabase.from("congress_trades").upsert(houseRows as any, {
        onConflict: "politician,ticker,transaction_date,transaction_type,amount_range",
        ignoreDuplicates: true,
      });

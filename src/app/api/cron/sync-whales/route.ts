...
export async function POST(request: Request) {
  try {
    ...
    const { error, count } = await supabase.from("whale_transactions").upsert(rows as any, {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: "Failed to sync whale transactions" }, { status: 500 });
    }
